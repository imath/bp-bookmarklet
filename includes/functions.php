...
/**
 * Handles activation to create a directory page
 *
 * @uses bp_core_get_directory_page_ids() to get the list of pages
 * @uses bkmklet_get_slug() to get the plugin's slug
 * @uses wp_insert_post() to write a new WordPress page
 * @uses bkmklet_get_name() to get the plugin's name
 * @uses bp_core_update_directory_page_ids to update the list of pages
 * @uses bp_update_option to store the plugin's version in root blog
 * @uses bkmklet_get_version() to get the plugin's version
 */
function bkmklet_activation() {
	// let's check for BP Bookmarklet page in directory pages first !
	$directory_pages = bp_core_get_directory_page_ids();
	$bkmklet_slug = bkmklet_get_slug();
	
	// if activity component is not active then stop everything!
	if( empty( $bkmklet_slug ) )
		return false;

	if( empty( $directory_pages[$bkmklet_slug] ) ) {
		// let's create a page and add it to BuddyPress directory pages
		$bkmklet_page_content = __( 'BP Bookmarklet uses this page, please leave it as is. It will not show in your navigation bar.', 'bp-bookmarklet' );

		$bkmklet_page_id = wp_insert_post( array( 
												'comment_status' => 'closed', 
												'ping_status'    => 'closed', 
												'post_title'     => bkmklet_get_name(),
												'post_content'   => $bkmklet_page_content,
												'post_name'      => $bkmklet_slug,
												'post_status'    => 'publish', 
												'post_type'      => 'page' 
												) );
		
		$directory_pages[$bkmklet_slug] = $bkmklet_page_id;
		bp_core_update_directory_page_ids( $directory_pages );
	}
	
	bp_update_option( 'bp-bookmarklet-version', bkmklet_get_version() );
}

/**
 * Handles deactivation to delete the directory page
 *
 * @uses bp_core_get_directory_page_ids() to get the list of pages
 * @uses bkmklet_get_slug() to get the plugin's slug
 * @uses wp_delete_post() to delete the WordPress page
 * @uses bp_core_update_directory_page_ids to update the list of pages
 * @uses bp_delete_option to remove the plugin's version
 */
function bkmklet_deactivation() {
	$directory_pages = bp_core_get_directory_page_ids();
	$bkmklet_slug = bkmklet_get_slug();
	...
	if( !empty( $directory_pages[$bkmklet_slug] ) ) {
		...
		$bkmklet_page_id = $directory_pages[$bkmklet_slug];
		wp_delete_post( $bkmklet_page_id, true );
		
		unset( $directory_pages[$bkmklet_slug] );
		bp_core_update_directory_page_ids( $directory_pages );
	}
	
	bp_delete_option( 'bp-bookmarklet-version' );
}
